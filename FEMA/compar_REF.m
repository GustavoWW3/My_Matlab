  function compar_REF(dir_name,storm_nam,date,type,LO_RES,HI_RES)
%
%  RE Jensen (02/2011)
%
...
%filelowd = ['../WAM_4_5_1C/Refraction/',dir_name,'/',storm_nam,'_',LO_RES,'.onlns'];
filelowd = ['../WAM_4_5_1C/Storm85/REF/',dir_name,'/',storm_nam,'-',LO_RES,'.onlns'];
a = load(filelowd);

%filehig= ['../../Storm_18sec/WAM_4_5_1C/',dir_name,'/',storm_nam,'-',date,'-',type,'-',HI_RES,'-05D.onlns'];
%filehigd = ['../WAM_4_5_1C/No_Refraction/',dir_name,'/',storm_nam,'_',HI_RES,'.onlns'];
filehigd = ['../WAM_4_5_1C/Storm85/NO_REF/',dir_name,'/',storm_nam,'-',HI_RES,'.onlns']
b = load(filehigd);
...
grid
ylabel('Ref H_{mo}  [m]','FontWeight','Bold');
xlabel('No Ref H_{mo}  [m]','FontWeight','Bold');
axis([0, hmax, 0, hmax]);
axis('square');
...
textslp=['H_{Ref} = ',num2str(chs),' \bullet H_{No Ref}'];
...
legend(H,'No Ref','Ref','Location','Northwest');
subplot(4,1,2)
plot(stns,bias_a,'.r-')
