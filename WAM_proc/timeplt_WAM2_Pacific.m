function timeplt_WAM2_Pacific(yrin,mntin,res,wt1,wt2)
%
...
%buoylocs = zeros(totlocs);

for kkk=1:totlocs
    filename =filesb(kkk,1).name;
...
%
    ...
    a = load(filename);
    totobsB=size(a,1);
    air_sea = zeros(totobsB,1);
    %windB = zeros(totobsB,1);
    
    %
    %  Now Load in the ENTIRE WAM oneline file
    %

    %Btot = zeros(10000,100,nw);   
    for ii = 1:nw
        filename2 = filesw(ii,1).name;
        fid = fopen(filename2,'r');
        if strcmp(filename2(1:3),'WAM')
            AAtot = textscan(fid,'%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f');
        else
            CCtot = textscan(fid,'%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f');
            qq = CCtot{4} ~= 46013;
        end
...
%             end
%         else
            for jj = 1:19%size(AAtot,2)
                if exist('CCtot','var')
                    AAtot{jj} = CCtot{jj}(qq);
                end
                Btot(:,jj,ii) = AAtot{jj};
            end
%        end
...
%         maxxx=ceil(max(max(timwam),max(timbuoy)));
        ...
        %
        titcharpt1='Pac Hindcast Study [WW3CY451C vs WW3-ST4]   ';
        titcharpt15=['                     (Res = ',num2str(res),'\circ)  [ ',lonM,'\circ / ',latM,'\circ ]'];
        ...
        for ii = 1:6
            ...
            if ii == 1
                ...
                hleg=legend(H,'BUOY',wtype{1},wtype{2});
        ...
        if nw == 1
           ...
           H=plot(timbuoy,comp(1).y2,'r.',timwam,comp(1).y1(:,1),'b-', ...
               timwam,comp(1).y1(:,2),'k-','MarkerSize',10,'LineWidth',1);
        end
        xticks=get(gca,'XTick');
        xtickdiff = max(timwam) - min(timwam);%xticks(end) - xticks(1);
        %xticks = [xticks(1):ceil(xtickdiff/8):xticks(end)];
        ...
        if kkk == totlocs
           for iii = 1:3
              ...
              for zzz = 1:aaa(iii)-1
                 ...
                 if zzz == 1
                    title1 = ['Pac Hindcast Study [WAMCY451C vs WW3-ST4]   '];
                    title2 = ['                                  (Res = ',num2str(res),'\circ)'];
        ...
        ylabel(name1);
        xlabel(name2);
        titchar1=['WAM4.5.3',' at ',int2str(buoy),' ',montit,' ' ,YR1];
        if ii == 1
            title(titchar1);
        end
        ...
        ylabel(name1)
        xlabel(name2)
        titchar2=['WAM4.5.3',' vs ',int2str(buoy),' Q-Q ',montit,' ' ,YR1];
        if ii == 1
            title(titchar2);
        end
