function map_movie_STCL3_new(pars,ice_coverin,storm,...
    track,modelnm)
%
%
...
filen = ls('c*0');
for zz = 1:size(filen,1)
    FILE1 = fopen(filen(zz,:));  
    field=fscanf(FILE1,'%10f',[nx,ny]);
    fclose(FILE1);
    FIELD{zz}=flipud(field');
    maxfld=max(max(max(FIELD{zz})),maxfld);
end
    titlefld1= fieldname;
    titlefld2= displname;
    titfile=filename;
...
%
%fileout1=[titfile,'-',modelnm,track,'-',storm]
...
%      in=FIELD==0;
  ...
  'Quality',95);
    ...
    for zz = 336:2:480%size(filen,1)
    for jj = 1:nx
        ll = find(FIELD{zz}(:,jj) < v(7) & FIELD{zz}(:,jj) > v(1));
        FIELD{zz}(ll,jj) = v(7);
    end
    ...
    [CMCF,hh]=m_contourf(x,y,FIELD{zz},v);
    hold on
    caxis([0,RANGMM]);
...
%     end
    ...
    %
    %textstg1=[titlefld2,sprintf('%5.2f',maxfld),' [',unts{1},']'];
...
titlnam1B=['  ',titlefld1,'  RESULTS:   ',track];
    ...
    textstg3=['DATE: ',filen(zz,2:end-2)];
    titlnam1=[{titlnam1A};{titlnam1B};{textstg3}];
    %textstrt=[{textstg1};{textstg3}];
    %
...
%         end
    ...
    textcolbr=[titlefld1,'  [',unts{1},']'];
    text(0.42,-.116,textcolbr,'FontWeight','bold','FontSize',8,'units','normalized');
    title(titlnam1,'FontWeight','bold');
    %text(legboxx,legboxy,textstg3,'FontWeight','bold','FontSize',10,'units','normalized','BackgroundColor','w');
    %
