function [se, sd] = stat_ftn(a,b,buoy,YRMT,montit,YR1,titdom,filedom,wtype);
%
...
se(4,:) = [xqb,yqb,bq,aq,rq,sq,ssq,rcq,csq,cpq,slq,iq,srq,urq,rsq,saq,sbq,nm];
 ...
 str = ['stats-PAC',int2str(buoy),'-',YRMT,'-',wtype,'.txt'];
 headstr=['          For Date = ',montit,' ',YR1,'   Domain = Pacific', '\n'];
 fid = fopen(str,'w');
 fprintf(fid,headstr);
 fprintf(fid,int2str(buoy));                                                    
 fprintf(fid,'  x-bar   y-bar    bias   abs(err) rms(err) S.I. S.S.  ');
 fprintf(fid,'corr  sym r  prin r \n');                                
 fprintf(fid,'   U  %6.2f  %6.2f  %6.2f  ',xub,yub,bu);                     
 fprintf(fid,' %6.2f   %6.2f   %3.0f',au,ru,su) ;                         
 fprintf(fid,' %5.2f %5.2f %6.2f ',ssu,rcu,csu);                             
 fprintf(fid,'%6.2f  \n',cpu);  
 fprintf(fid,'  HS  %6.2f  %6.2f  %6.2f  ',xhb,yhb,bh);                     
 fprintf(fid,' %6.2f   %6.2f   %3.0f',ah,rh,sh);                          
 fprintf(fid,' %5.2f %5.2f %6.2f ',ssh,rch,csh);                             
 fprintf(fid,'%6.2f  \n',cph); 
 fprintf(fid,'  TP  %6.2f  %6.2f  %6.2f  ',xpb,ypb,bp);                     
 fprintf(fid,' %6.2f   %6.2f   %3.0f',ap,rp,sp);                          
 fprintf(fid,' %5.2f %5.2f %6.2f ',ssp,rcp,csp);                             
 fprintf(fid,'%6.2f  \n',cpp); 
 fprintf(fid,'  TM  %6.2f  %6.2f  %6.2f  ',xqb,yqb,bq);                     
 fprintf(fid,' %6.2f   %6.2f   %3.0f',aq,rq,sq);                          
 fprintf(fid,' %5.2f %5.2f %6.2f ',ssq,rcq,csq);                             
 fprintf(fid,'%6.2f  \n\n',cpq); 

 fprintf(fid,int2str(buoy)) ;                                     
 fprintf(fid,'  slope  intcpt   sys-err usys-err  rmse-sl'); 
 fprintf(fid,'  stde-a  stde-b    pts \n'); 
 fprintf(fid,'   U  %6.2f  %6.2f   %6.3f  ',slu,iu,sru);       
 fprintf(fid,' %6.3f   %6.3f   %6.4f ',uru,rsu,sau);             
 fprintf(fid,' %6.4f %6.0f \n',sbu,nu);   
 fprintf(fid,'  HS  %6.2f  %6.2f   %6.3f  ',slh,ih,srh);        
 fprintf(fid,' %6.3f   %6.3f   %6.4f ',urh,rsh,sah);             
 fprintf(fid,' %6.4f %6.0f \n',sbh,nh);    
 fprintf(fid,'  TP  %6.2f  %6.2f   %6.3f  ',slp,ip,srp);        
 fprintf(fid,' %6.3f   %6.3f   %6.4f ',urp,rsp,sap) ;            
 fprintf(fid,' %6.4f %6.0f \n',sbp,np); 
 fprintf(fid,'  TM  %6.2f  %6.2f   %6.3f  ',slq,iq,srq);        
 fprintf(fid,' %6.3f   %6.3f   %6.4f ',urq,rsq,saq);             
 fprintf(fid,' %6.4f %6.0f \n\n',sbq,nm);                    
...
fprintf(fid,int2str(buoy));
fprintf(fid,'      scl-xd  sigx scl-yd sigy vec-xd vec-yd diff sigma ');
fprintf(fid,'rho    phase   pts\n');
fprintf(fid,'Wind Dir  %6.1f %5.1f %6.1f',xdb,sigx,ydb);
fprintf(fid,'%5.1f %6.1f %6.1f',sigy,avx,avy);
fprintf(fid,'%5.1f %5.1f %5.2f ',bd,rd,rho);
fprintf(fid,'%6.1f %5.0f\n\n\n',pha,nu);
%
...
fprintf(fid,'      scl-xd  sigx scl-yd sigy vec-xd vec-yd diff sigma ');
fprintf(fid,'rho    phase   pts\n');
fprintf(fid,'Wave Dir  %6.1f %5.1f %6.1f',xdbwv,sigxwv,ydbwv);
fprintf(fid,'%5.1f %6.1f %6.1f',sigywv,avxwv,avywv);
fprintf(fid,'%5.1f %5.1f %5.2f ',bdwv,rdwv,rhowv);
fprintf(fid,'%6.1f %5.0f\n\n\n',phawv,nwavd);
