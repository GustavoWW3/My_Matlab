function timeplt_WAM_stclair(yrin, mntin, domain)
%
...
%for kkk=1:totlocs
    ...
    if ~isempty(istn);
        ...
        %
        titcharpt1=['Lake St. Clair 25-yr Hindcast Study [OWI Winds]   '];
        ...
        orient('Tall')
        subplot(totlocs+2,3,3*kkk+4:3*kkk+6)
        ...
        if kkk == totlocs
            for ii = 1:totlocs
                subplot(totlocs+2,3,3*ii+4:3*ii+6)
                ylimmx = [0,ylimmax];
                set(gca,'Ylim',ylimmx);
                text(max(xlimxx)-3.0,ylimmax-0.08*ylimmax,buoylocs(ii,:))
                if ii == 1
                    title1 = ['Lake St. Clair 25-yr Hindcast Study [OWI Winds]   '];
                    title2 = [titdom2,'   WAM4.5.1C '];
                    title3 = ['Wave Heights H_{mo} (m)'];
                    titletot = [{title1;title2;title3}];
                    text(timwam(1),ylimmax*3.5,titletot,'FontWeight','bold','EdgeColor','black','LineWidth',2);
                    hleg=legend(H,'BUOY','WAM');
                    po=get(hleg,'position');
                    po=[0.3 0.74 0.08 0.03];
                    set(hleg,'position',po);
