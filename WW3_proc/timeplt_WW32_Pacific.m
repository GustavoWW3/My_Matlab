function timeplt_WW32_Pacific(yrin, mntin,res,nw,test)
%
...
buoyset1 = [51001;51002;51003;51004;51028;00000];
buoyset2 = [46002;46005;46006;46047;46059;00000];
buoyset3 = [46001;46066;46035;46071;46072;46075];
buoyset4 = [46001;46005;51001;00000;00000;00000];
...
xlonwc = [190.00 216.00 175.00 110.00];
xlonec = [215.00 246.00 216.00 300.00];
xlatsc = [-6.00 25.00 40.00 -64.00];
xlatnc = [30.00 50.00 64.00 64.00];
shr_x(1,1) = xlonw;
shr_x(2,1) = xlone;
shr_x(3,1) = xlone;
shr_x(4,1) = xlonw;
...
xtitl=['Days in ',montit,'  ',int2str(yr)];
    ...
    %
    %  Start Looking for the WW3 Results
...
filesb = dir('n*.onlns');
filess = dir('s*.mat');
filest = [filesb;filess];
%filesw = dir('WW3-PACIFIC*.onlns');
totlocs=size(filest,1);
aaa(1:4) = 1;
%for zz = 1:size(filesw,1)
for kkk=1:totlocs
    filename = getfield(filest(kkk,1),'name');
...
%for kkk=1:totlocs
    ...
    elseif strcmp(filename(1),'s')
        load(filename);
        a = cdipread_2_onlns(aa,buoy);
    end
    totobsB=size(a,1);
    %
    %  Now Load in the ENTIRE WW3 oneline file
    %
    %for ii = 1:nw
        %filename2 = getfield(filesw(ii,1),'name');
        %fid = fopen(filename2,'r');
        filename2n = ['ST',buoylocs(kkk,:),'.onlns'];
        filename2 = ls(filename2n);filename2 = filename2(1:end-1);
...
%        %AAtot = textscan(fid,'%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f');
%          if size(AAtot{2},1) > size(AAtot{3},1)
%             Btot(:,:,1) = AAtot{1}(1:end-1);
%             Btot(:,:,2) = AAtot{2}(1:end-1);
...
%         fclose(fid);
        Btot = AAtot;
            ...
            fprintf(1,'Model did not output Station: %1s\n',buoylocs(kkk,:));
            continue
        end
...
%     if filename2(12) == 'P'
%         wtype{ii} = 'OWI';
%     elseif filename2(23) == 'C'
%         wtype{ii} = 'CFSR';
%     end
    %end
       %
    %  Split out the proper WW3 Save pt for the buoy comparison
    %
    ...
    %if ~isempty(istn);
        ...
        %
        for jjj = 1:nw
            filestats{jjj} = ['stats',int2str(buoy),'-',YRMT,'-',wtype{jjj},'.asc'];
        ...
        %  Set the time
        %   1.  Buoy and then WW3
        %       For WW3 need to split out the date into Year,Month,Day,Hour
        ...
        latM=num2str(B1(1,3,1));
        lonM=num2str(B1(1,4,1));
        LONout=B1(1,4,1);
        LATout=B1(1,3,1);
        DEPout=a(1,13);
        depb=num2str(a(1,13));
        ...
        titcharpt15=['WaveWatch III ',test,' (Res = ',num2str(res),'\circ)  '];
        titcharpt17=['   [ ',lonM,'\circ / ',latM,'\circ ]'];
        titcharpt2=['NDBC = ',int2str(buoy),' [',lonb,...
                '\circ / ',latb,'\circ], Depth = ',depb,'m'];
        %titcharpt25=['at Depth = ',depb,'m'];
        titchar1=[{titcharpt1}];
        titchar2=[{titcharpt2;titcharpt15;titcharpt17}];
        ...
        for jjj=1:nw
            tppfM(:,jjj)=B1(:,11,jjj);
            itpM(:,jjj)=find(B1(:,11,jjj) > 0);
            tppM2(:,jjj) = B1(itpM(:,jjj),11,jjj);
        end
        
        tppmax=ceil(max(max(max(tppM2)),max(tppfB(itpB))));
        ...
        for ii = 1:6
            ...
            if nw == 1
                H=plot(timbuoy,comp(ii).y2,'r.',timwam,comp(ii).y1(:,1),'b-','MarkerSize',8,'LineWidth',1);
            else
        ...
        %  Print out the file 
        %
        ffout = ['PAC',int2str(buoy),'-',YRMT];
        %print(gcf,'-dpdf','-r600',ffout)
        set(gcf,'Renderer','Zbuffer');
        saveas(gcf,ffout,'png')
        clf
        
        lonmp2(kkk,1) = str2double(lonb(1,:));
        latmp2(kkk,1) = str2double(latb(1,:));
        if lonmp2(kkk,1) < 0
            lonmp2(kkk,1) = 360 + lonmp2(kkk,1);
        end
        ...
        if buoy == buoyset1(ll,:)
            ...
            bbb{1}(aaa(1)) = kkk;
            aaa(1) = aaa(1) + 1;
        elseif buoy == buoyset2(ll,:)
            figure(3)
            cof = 2;
            ppp = aaa(2);
            bbb{2}(aaa(2)) = kkk;
            aaa(2) = aaa(2) + 1;
        elseif buoy == buoyset3(ll,:)
            figure(4)
            cof = 3;
            ppp = aaa(3);
            bbb{3}(aaa(3)) = kkk;
        ...
        if cof > 0
        orient('Tall')
        subplot(8,3,3*ppp+4:3*ppp+6)
        if nw == 1
           H=plot(timbuoy,comp(1).y2,'r.',timwam,comp(1).y1(:,1),'b-','MarkerSize',8,'LineWidth',1);
        else
        ...
        grid
        ylabel(comp(1).name);
        ylimmax(cof) = max(ylimmax(cof),comp(1).max);
        end
        if kkk == totlocs
           for iii = 1:3
              figure(iii+1)
              subplot(8,3,[2 6])
              m_proj(project_in,'long',[xlonwc(iii) xlonec(iii)],'lat',[xlatsc(iii) xlatnc(iii)]);
              %m_patch(shr_x,shr_y,[.0 .5 .0]);
              m_gshhs_i('patch',[.0 .5 .0],'edgecolor','k');
              hcst=findobj(gca,'Tag','m_gshhs_i');
              set(hcst,'HandleVisibility','off');
              ...
              set(gca,'Position',[0.45 0.74 0.7 0.25]);
              for zzz = 1:aaa(iii)-1
                 m_text(lonmp2(bbb{iii}(zzz)),latmp2(bbb{iii}(zzz)),buoylocs(bbb{iii}(zzz),:),'fontsize',8,'HorizontalAlignment','right', ...
                    'VerticalAlignment','Bottom','backgroundcolor',[1 1 1]);
                 m_plot(lonmp2(bbb{iii}(zzz)),latmp2(bbb{iii}(zzz)),'r.','MarkerSize',12)
              end
              
              hold off
              for zzz = 1:aaa(iii)-1
                 subplot(8,3,3*zzz+4:3*zzz+6)
                 ylimmx = [0,ylimmax(iii)];
                 set(gca,'Ylim',ylimmx);
                 text(max(xlimxx)-3.0,ylimmax(iii)-0.08*ylimmax(iii),buoylocs(bbb{iii}(zzz),:))
                 if zzz == 1
                    title1 = ['        Pacific Hindcast Study [OWI Winds]   '];
                    title2 = ['                WaveWatch III ',test,' (Res = ',num2str(res),'\circ)'];
                    title3 = ['                 Wave Heights H_{mo} (m) Analysis'];
                    titletot = [{title1}];
                    titletot2= [{title2;title3}];
                    text(timwam(1),ylimmax(iii)*4,titletot,'FontWeight','bold','Fontsize',12);
                    text(timwam(1),ylimmax(iii)*3,titletot2,'FontWeight','bold','Fontsize',11);
                    if nw == 2
                     hleg=legend('BUOY',wtype{1},wtype{2});
                    else
                        hleg = legend('BUOY',wtype{1});
            ...
            xlabel(xtitl)
            ff15 = ['PAC','buoy_comp_set_',num2str(iii),'-',YRMT];
            %print(gcf,'-dpng','-r600',ff15)
            saveas(gcf,ff15,'png')
            clf
           end
          end
        ...
        %
        %   Output Time pair file 
        save(['Pac-',int2str(buoy),'-',YRMT,'.mat'],'AB','AM');
        ...
        axis('square')
        name1 = [wtype{jjj},' WW3 ',comp(ii).name];
        name2 = ['Buoy ',comp(ii).name];
        ylabel(name1);
        xlabel(name2);
        titchar1=['WW3',' at ',int2str(buoy),' ',montit,' ' ,YR1];
        if ii == 1
            title(titchar1);
        end
        ...
        ylabel(name1)
        xlabel(name2)
        titchar2=['WW3',' vs ',int2str(buoy),' Q-Q ',montit,' ' ,YR1];
        if ii == 1
            title(titchar2);
        end
        dist(1)= {['%tile  ','BUOY','  WW3']};
        ...
        ff3 = ['scat-PAC',int2str(buoy),'-',YRMT,'-',wtype{jjj},'-p1'];
        %print(gcf,'-dpdf','-r400',ff3);
        saveas(gcf,ff3,'png');
        clf
        ...
        if ~isempty(iwsg)
            ...
            ylabel(yname);
            xlabel('Buoy WS');
            titchar1=[wtype{jjj},' at ',int2str(buoy),' ',montit,' ' ,YR1];
            title(titchar1);
            ...
            ylabel(yname);
            xlabel('Buoy WS');
            titchar2=[wtype{jjj},' vs ',int2str(buoy),' Q-Q ',montit,' ' ,YR1];
            title(titchar2);
            %
            %    
            subplot(3,2,3)
        ...
        %  Write to Stats file
        %
        fprintf(fidstats{jjj},'  %4.0f  %2.0f  %1.0f  %5.0f',yrin,mntin,4,buoy);
        fprintf(fidstats{jjj},'  %7.2f  %7.2f  %6.0f',LONout,LATout,DEPout);
        ...
        fprintf(fidstats{jjj},'  %6.2f  %6.2f  %6.2f  %6.2f %6.2f \n',qqM(99),qqM(95),qqM(90),qqM(85),qqM(80));
        %
        fprintf(fidstats{jjj},'  %4.0f  %2.0f  %1.0f  %5.0f',yrin,mntin,5,buoy);
        fprintf(fidstats{jjj},'  %7.2f  %7.2f  %6.0f',LONout,LATout,DEPout);
        ...
        %           1.  IF the number of observations arValidatione zero then skip plotting.
        %
        if isempty(iwadg) == 0
            ...
            clear str; 
            fprintf(fidstats{jjj},'  %4.0f  %2.0f  %1.0f  %5.0f',yrin,mntin,6,buoy);
            fprintf(fidstats{jjj},'  %7.2f  %7.2f  %6.0f',LONout,LATout,DEPout);
        ...
        ff2 = ['scat-PAC',int2str(buoy),'-',YRMT,'-',wtype{jjj},'-p2'];
        %print(gcf,'-dpdf','-r400',ff2);
        saveas(gcf,ff2,'png')
        clf
        fclose(fidstats{jjj});
        end
